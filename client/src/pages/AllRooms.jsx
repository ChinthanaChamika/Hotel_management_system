...
const RadioButtton = ({label, selected = false, onChange = ()=> { }}) =>{
    return(
        <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
                ...
                {roomsDummyData.map((room)=>(
                    <div key ={room._id}className="flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0">
                        ...
                        <div className="md:w-1/2 flex flex-col gap-2">
                            ...
                            <p onClick = {()=>{navigate(`/rooms/${room._id}`); scrollTo (0,0)} }src = {room.images[0]} className="text-gray-800 text-3xl font-playfair cursor-pointer">{room.hotel.name}</p>
                ...
                <div className={`${openFilters ? 'h-auto' : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
                    <div className="px-5 pt-5">
                        ...
                        {sortOptions.map((option,index)=>( 
                            <RadioButtton key={index} label={option}/>
                        ))}
                    </div>
                </div>
            </div>
